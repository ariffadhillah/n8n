...
def extract_email(website: str) -> str:
    ...
    s = grab(website)
    if emails: return next(iter(emails))
    if s:
        candidates = []
        for a in s.find_all("a", href=True):
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        #     if cnt == last or cnt >= MAX_RESULTS: break
        #     last = cnt
        ...
        # Scroll sampai muncul teks "You've reached the end of the list."
        feed = page.locator("div[role='feed']")
        while True:
            await feed.evaluate("el => el.scrollTo(0, el.scrollHeight)")
            await page.wait_for_timeout(1000)

            # cek kalau sudah muncul "You've reached the end of the list."
            try:
                await page.wait_for_selector("text=You've reached the end of the list.", timeout=2000)
                print("✅ Sampai akhir list.")
                break
            except:
                # kalau belum ketemu, lanjut scroll
                continue

        # setelah scroll selesai, ambil semua card
        cards = page.locator(".hfpxzc, div[role='article']")
        total = await cards.count()
        print("Total:", total)
